...
# Task generator
def create_task():
	global n_shapes,m_holdout,all_shapes,train_shapes,test_shapes,train_set_size,test_set_size,train_proportion
	...
	if m_holdout == 0:
		...
		if train_set_size + test_set_size > total_trials:
			...
			log.info('Desired training set size (' + str(train_set_size) + ') and test set size (' + str(test_set_size) + ') combined are larger than total number of possible trials for this task (' + str(total_trials) + ')...')
		...
		if train_set_size > total_trials:
			log.info('Desired training set size (' + str(train_set_size) + ') is larger than total number of possible training trials for this task (' + str(total_trials) + ')...')
